...
def create_points_chart(game_data):
    ...
    points = [float(points) for _, points in game_data]
    ...
    # Create line plot
    ax.plot(dates, points, '-', alpha=0.5)

    # Create scatter plot
    scatter = ax.scatter(dates, points, c=points, cmap='viridis', s=50)
    
    # Calculate and plot trend line
    z = np.polyfit(dates_num, points, 1)
    p = np.poly1d(z)
    ax.plot(dates, p(dates_num), "r--", alpha=0.8, label='Trend')
    ...
    # Create tooltip HTML
    tooltip_html = [f"<div>Date: {date.strftime('%Y-%m-%d')}<br>Points: {points}</div>" for date, points in zip(dates, points)]
